...
export function BreadcrumbNav({ items, onAdd }: BreadcrumbNavProps) {
  return (
    <motion.div
      ...
      className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-slate-50 to-white border-b border-slate-200 overflow-x-auto"
    >
      {items.map((item, idx) => (
        <div key={`${item.label}-${idx}`} className="flex items-center gap-2 whitespace-nowrap">
          ...
          <motion.button
            ...
            className="text-sm font-medium text-slate-600 hover:text-slate-900 px-2 py-1 rounded transition-colors"
          >
      ...
      {onAdd && (
        <motion.button
          ...
          whileTap={{ scale: 0.95 }}
          onClick={onAdd}
          className="ml-auto p-1 rounded hover:bg-slate-100 text-slate-600 transition-colors"
...
export function TabNavigation({ tabs, activeTabId, onTabChange }: TabsProps) {
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      transition={{ duration: 0.2 }}
      className="flex items-center gap-1 px-2 py-2 border-b border-slate-200 overflow-x-auto bg-white"
    >
      {tabs.map((tab) => (
        <motion.button
          key={tab.id}
...
export function Sidebar({ isOpen, onClose, title, children }: SidebarProps) {
  return (
    <motion.div
      ...
      className={`fixed left-0 top-0 h-full w-80 bg-white border-r border-slate-200 shadow-lg z-40 ${
