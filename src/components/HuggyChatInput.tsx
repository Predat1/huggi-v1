...
function TagPill({ tag, onRemove }: { tag: ChatTag; onRemove: () => void }) {
  return (
    <div
...
export default function HuggyChatInput({
  ...
  return (
    <div className={`flex flex-col items-center gap-2 w-full ${className}`}>
      ...
      >
        ...
        <div className="px-4 pt-3 pb-1">
          <textarea
            ...
            rows={1}
            className="w-full bg-transparent text-white text-[17px] font-medium placeholder-white/25 resize-none focus:outline-none leading-relaxed disabled:opacity-50"
            style={{ caretColor: '#7F77DD', minHeight: '28px' }}
          />
        </div>
        ...
        <div className="flex items-center justify-between px-3 pb-3 pt-1">
          ...
          <div className="flex items-center gap-0.5">
            ...
            <button
              type="button"
              className="w-8 h-8 rounded-xl flex items-center justify-center text-white/40 hover:text-white/70 hover:bg-white/8 transition-all"
              aria-label="Attach"
            >
              <Plus size={18} />
            </button>
            ...
            <button
              type="button"
              className="flex items-center gap-1.5 px-3 h-8 rounded-xl hover:bg-white/8 transition-all group"
            ...
            >
              ...
              <span className="text-white/80 text-xs font-semibold group-hover:text-white transition-colors">{modelLabel}</span>
            ...
            <button
              type="button"
              className="w-8 h-8 rounded-xl flex items-center justify-center text-white/40 hover:text-violet-400 hover:bg-violet-500/10 transition-all"
              aria-label="Magic"
            >
              <Sparkles size={15} />
            </button>
          ...
          <div className="flex items-center gap-2">
              ...
              {isListening ? (
                ...
                <span className="flex items-end gap-[2px] h-4">
                  {[3, 5, 7, 5, 3].map((h, i) => (
                    <span
                      key={i}
                      className="w-[2px] rounded-full bg-violet-400 animate-bounce"
            ...
            <button
              ...
              onClick={handleSend}
              disabled={!canSend}
              className={`w-9 h-9 rounded-xl flex items-center justify-center transition-all font-bold shadow-lg ${
                canSend
      ...
      {/* Disclaimer */}
      {disclaimer && (
        <p className="text-[11px] text-white/25 text-center leading-relaxed">{disclaimer}</p>
      )}
    </div>
  );
}
